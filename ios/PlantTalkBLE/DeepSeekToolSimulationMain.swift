...
/// An opt-in command-line regression runner. It links the same production
/// SQLite database, tool executor and OpenAI-compatible text adapter as the
/// iOS app. Credentials are read only from the invoking environment.
@main
@MainActor
struct DeepSeekToolSimulationMain {
    static func main() async {
        do {
            ...
            if !report.issues.isEmpty {
                fputs(
                    "DeepSeek simulation completed with \(report.issues.count) validation issue(s).\n",
                    stderr
                )
                exit(2)
            }
        } catch {
            fputs("DeepSeek simulation failed: \(error.localizedDescription)\n", stderr)
            exit(1)
        }
    }

    private static func run() async throws -> SimulationReport {
        let environment = ProcessInfo.processInfo.environment
        guard let apiKey = environment["DEEPSEEK_API_KEY"], !apiKey.isEmpty else {
            throw SimulationError.missingAPIKey
        }

        var calendar = Calendar(identifier: .gregorian)
        calendar.timeZone = try unwrap(TimeZone(identifier: "Asia/Shanghai"))
        let now = try unwrap(calendar.date(from: DateComponents(
        ...
        let historyStart = try unwrap(calendar.date(from: DateComponents(
            timeZone: calendar.timeZone,
            year: 2026,
            month: 6,
            day: 29,
        ...
        let configuration = AIConfiguration(
            baseURL: try unwrap(URL(string: environment["DEEPSEEK_BASE_URL"] ?? "https://api.deepseek.com")),
        ...
        let scenarios = [
            Scenario("现在的温度、空气湿度和光照怎么样？", .currentSensorReading),
            Scenario("最近一次保存的历史记录是什么时候？温度和土壤 ADC 原始值分别是多少？", .latestHistoricalReading),
            Scenario("今天截至现在，温度和空气湿度的平均值分别是多少？", .sensorSummary),
            Scenario("昨天温度的最低值和最高值是多少？", .sensorSummary),
            Scenario("最近 24 小时的光照整体情况如何？平均值、最低值和最高值分别是多少？", .sensorSummary),
            Scenario("过去 7 天的平均温度和平均空气湿度是多少？", .sensorSummary),
            Scenario("今天上午 9:00 到 10:00，温度每 5 分钟是怎样变化的？", .sensorSeries, .fiveMinutes),
            Scenario("昨天按小时看，温度与光照的变化趋势怎样？", .sensorSeries, .hour),
            Scenario("过去 7 天按天看，土壤 ADC 原始值的变化趋势怎样？", .sensorSeries, .day),
            Scenario("现在土壤湿度是百分之多少？如果不能可靠换算，请说明目前能确定的原始数据。", .currentSensorReading)
        ]

        let adapter = TextModelAdapter(client: .live())
        var traces: [ScenarioTrace] = []
        var issues: [String] = []
        ...
        for (offset, scenario) in scenarios.enumerated() {
            ...
            do {
                ...
                if normalizedAnswer.isEmpty {
                    validationIssues.append("模型没有生成最终自然语言回答")
                }
                if !invocations.contains(where: { $0.toolName == scenario.expectedTool.rawValue }) {
                    validationIssues.append("没有调用预期工具 \(scenario.expectedTool.rawValue)")
                }
                if let expectedGranularity = scenario.expectedGranularity,
                   !invocations.contains(where: {
                       $0.argumentsJSON.contains(
                        "\"granularity\" : \"\(expectedGranularity.rawValue)\""
                       )
                   }) {
                    validationIssues.append("没有使用预期粒度 \(expectedGranularity.rawValue)")
                }
                if offset == 9 {
                    if !(normalizedAnswer.contains("ADC") || normalizedAnswer.contains("原始")) {
                        validationIssues.append("没有说明土壤数据是 ADC 原始值")
                    }
                    if !["不能直接", "不可直接", "无法直接", "不能可靠", "无法可靠"]
                        .contains(where: { normalizedAnswer.contains($0) }) {
                        validationIssues.append("没有明确拒绝将土壤 ADC 直接换算为百分比")
                    }
                }
                ...
                traces.append(ScenarioTrace(
                    ...
                    toolInvocations: invocations.map(ToolTrace.init),
                    validationIssues: validationIssues
                ))
            } catch {
                let issue = "请求失败：\(error.localizedDescription)"
                issues.append("第 \(offset + 1) 题：\(issue)")
                traces.append(ScenarioTrace(
                    index: offset + 1,
                    question: scenario.question,
                    expectedTool: scenario.expectedTool.rawValue,
    ...
    private static func unwrap<T>(_ value: T?) throws -> T {
        guard let value else { throw SimulationError.invalidSetup }
        return value
    }

    private static func makeSyntheticDataset(
        from start: Date,
        through now: Date,
        calendar: Calendar
    ) -> SyntheticDataset {
    ...
    private static func simulatedValues(
        at date: Date,
        start: Date,
        calendar: Calendar
        ...
        // Day 8 has one watering event; values remain raw ADC readings.
        let soil = min(
            3_800,
            max(
                700,
                2_460 - elapsedDays * 68
                    + (elapsedDays >= 8 ? 510 : 0)
                    + sin(elapsedDays * 2.2) * 18
            )
        )
...
private struct Scenario {
    ...
    init(
        _ question: String,
        _ expectedTool: PlantToolName,
        _ expectedGranularity: SensorSeriesGranularity? = nil
    ) {
        self.question = question
        self.expectedTool = expectedTool
        self.expectedGranularity = expectedGranularity
    }
}
...
private struct ScenarioTrace {
    let index: Int
    let question: String
    let expectedTool: String
    let answer: String
    let toolInvocations: [ToolTrace]
    let validationIssues: [String]
}

private struct ToolTrace {
    let name: String
    let summary: String
...
private struct SimulationReport {
    let model: String
    let dataset: SyntheticDataset
    let traces: [ScenarioTrace]
    let issues: [String]
    ...
    var markdown: String {
        ...
        if issues.isEmpty {
            lines.append("> 自动校验通过：10 个问题均得到最终回答、调用了预期工具及粒度，并且第 10 题没有把未校准的土壤 ADC 误写成百分比。")
        } else {
            lines.append("> 自动校验发现问题：")
        ...
        for trace in traces {
            ...
            if !trace.validationIssues.isEmpty {
                lines.append("- 校验问题：\(trace.validationIssues.joined(separator: "；"))")
            }
            lines.append("")
